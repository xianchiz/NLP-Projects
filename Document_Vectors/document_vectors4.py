...
def main():
    ...
    PMI = np.zeros((len(terms), len(documents)))
    for term_idx, term in enumerate(terms):
        for document_idx, document in enumerate(documents):
            PMI[term_idx, document_idx] = log(
                (document.count(term) + 1) / len(document)
            ) - log((sum([document.count(t) for t in terms]) + 1) / len(document))
    X = PMI.T / np.linalg.norm(PMI.T, axis=1, keepdims=True)
